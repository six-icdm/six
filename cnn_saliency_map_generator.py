...
def get_by_class_saliency_six(image_path,
                              label,
                              operations,
                              model_name='densnet',
                              layers=[12],
                              ...
                              device='cuda',
                              use_mask=False):
    images, integrated_heatmaps1 = heatmap_of_layer_six(device, image_path, label,
                                                        layers,
                                                        model_name)
    images2, integrated_heatmaps2 = heatmap_of_layer_six(device, image_path, label,
                                                         [layers[0] - 1],
                                                         model_name)

    if operations[0] == 'six1':
        images, integrated_heatmaps1 = heatmap_of_layer_six(device, image_path, label,
                                                            layers,
                                                            model_name)
...
def heatmap_of_layer_six(device, image_path, label, layers, model_name):
    model = GradModel(model_name, feature_layer=layers[0])
    model.to(device)
    model.eval()
    model.zero_grad()
    ...
    full_tensor = get_noise_values_six(model, activations, label)
...
def get_noise_values_six(model, activation, label, device='cuda'):
    ...
    interpolated_activations = six_baseline(model, label, device, activation, perturb_activation)

    return interpolated_activations


def six_baseline(model, label, device, activations, pert_activation_last):
...
if __name__ == '__main__':
    ...
    operations = ['six2', 'fullgrad', 'ablation-cam', 'lift-cam', 'layercam', 'ig', 'blurig', 'guidedig',
                  'gradcam', 'gradcampp', 'x-gradcam']
    operations = ['six1']
    operations = ['six1', 'six2', 'six1_123']
    OPERATION_NUM = 3
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        img_dict = []
        for operation in operations:
            if operation.__contains__('six1'):
                t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                    get_by_class_saliency_six(image_path=image_path,
                                              label=label,
                                              ...
                                              device=device,
                                              use_mask=USE_MASK)
            elif operation.__contains__('six2'):
                t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                    get_by_class_saliency_six(image_path=image_path,
                                              label=label,
                                              operations=[operation],
