...
def get_six_vit(vit_ours_model, feature_extractor, device, index, image, layer_num):
    AGGREGATE_BEFORE = False

    attention_probs = []
    attention_grads = []

    output = vit_ours_model(image.unsqueeze(0).cuda())
    ...
    noise_tensor = torch.randn_like(attn).cuda()

    ab_t = get_alphas_from_timestamp(TIMESTEPS).cuda()
    baseline_attention = perturb_input(attn, TIMESTEPS, noise_tensor, ab_t)

    integrated_attn = get_interpolated_values(baseline_attention.cpu(), attn.cpu(),
                                              num_steps=ITERATION_STEPS)
    cams = []
    grads = []
    for iatn in integrated_attn:
        ...
        delta = ((iatn.detach().cpu() - baseline_attention.detach().cpu()) / ITERATION_STEPS)
        ...
        block_cam_tensor = torch.stack(block_cams)
        cams.append(block_cam_tensor)
        grads.append(torch.stack(block_grads))
    attention_probs.append(torch.stack(cams))
    attention_grads.append(torch.stack(grads))

    delta = (attn - baseline_attention) / ITERATION_STEPS

    integrated_attention = torch.stack(attention_probs) * F.relu(delta.squeeze().unsqueeze(0).unsqueeze(0))
...
def get_six_vit2(vit_ours_model, feature_extractor, device, index, image, layer_num):
...
def handle_transformers(model, operations, image, label, gt_image):
    ...
    for operation in operations:
        if operation == 'gae':
            ...
            img_dict.append({"image": im, "title": operation})
            ...
            img_dict.append({"image": im2, "title": operation})
        elif operation.__contains__(SIX):
            six_attribution1 = get_six_vit(model, [], device, label,
                                           image, 1)
            six_attribution2 = get_six_vit(model, [], device, label,
                                           image, 2)
            six_attribution = six_attribution1 + six_attribution2
            im3, score, heatmap_cv, blended_img_mask, heatmap, t = blend_transformer_heatmap(image, six_attribution)
            img_dict.append({"image": im3, "title": operation})
...
if __name__ == '__main__':
    ...
    methods = ['six', 'gae', 't-attr']
    methods = ['six']
    methods = ['six1', 'six2', 'six3']
